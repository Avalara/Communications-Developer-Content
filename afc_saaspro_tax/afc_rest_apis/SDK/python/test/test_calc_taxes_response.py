# coding: utf-8

"""
    SaasPro

    APIs to interface with communications tax engine.<br />The API requires Basic authentication.<br />Users with access to multiple clients must also set request header parameter for <code>client_id</code>.<br />Set <code>client_profile_id</code> to specify profile to be used for taxation.<br /><br />Effective January 1st, 2022 all Avalara products will be enforcing TLS 1.2 server-side. Please ensure that your implementation sets a minimal of TLS 1.2 encryption when making web requests to Avalara APIs.  # noqa: E501

    The version of the OpenAPI document: v2
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest
import datetime

import avalara.comms.rest.v2
from avalara.comms.rest.v2.models.calc_taxes_response import CalcTaxesResponse  # noqa: E501
from avalara.comms.rest.v2.rest import ApiException

class TestCalcTaxesResponse(unittest.TestCase):
    """CalcTaxesResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test CalcTaxesResponse
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # model = avalara.comms.rest.v2.models.calc_taxes_response.CalcTaxesResponse()  # noqa: E501
        if include_optional :
            return CalcTaxesResponse(
                inv = [
                    avalara.comms.rest.v2.models.invoice_result.InvoiceResult(
                        doc = '0', 
                        itms = [
                            avalara.comms.rest.v2.models.line_item_result.LineItemResult(
                                ref = '0', 
                                base = 1.337, 
                                txs = [
                                    avalara.comms.rest.v2.models.tax.Tax(
                                        bill = True, 
                                        cmpl = True, 
                                        tm = 1.337, 
                                        calc = 56, 
                                        cat = '0', 
                                        cid = 56, 
                                        name = '0', 
                                        exm = 1.337, 
                                        lns = 56, 
                                        min = 1.337, 
                                        pcd = 56, 
                                        taxpcd = 56, 
                                        rate = 1.337, 
                                        sur = True, 
                                        tax = 1.337, 
                                        lvl = 56, 
                                        tid = 56, 
                                        usexm = True, 
                                        notax = True, 
                                        trans = 56, 
                                        svc = 56, 
                                        chg = 1.337, )
                                    ], 
                                err = [
                                    avalara.comms.rest.v2.models.error.Error(
                                        code = 56, 
                                        msg = '0', )
                                    ], )
                            ], 
                        summ = [
                            avalara.comms.rest.v2.models.summarized_tax.SummarizedTax(
                                max = 1.337, 
                                min = 1.337, 
                                tchg = 1.337, 
                                calc = 56, 
                                cat = '0', 
                                cid = 56, 
                                name = '0', 
                                exm = 1.337, 
                                lns = 56, 
                                pcd = 56, 
                                taxpcd = 56, 
                                rate = 1.337, 
                                sur = True, 
                                tax = 1.337, 
                                lvl = 56, 
                                tid = 56, 
                                usexm = True, 
                                notax = True, 
                                trans = 56, 
                                svc = 56, 
                                chg = 1.337, )
                            ], 
                        err = [
                            avalara.comms.rest.v2.models.error.Error(
                                code = 56, 
                                msg = '0', )
                            ], 
                        incrf = avalara.comms.rest.v2.models.reporting_information.ReportingInformation(
                            acct = '0', 
                            custref = '0', 
                            invn = '0', 
                            bcyc = '0', 
                            ccycd = '0', 
                            ccydesc = '0', ), )
                    ], 
                err = [
                    avalara.comms.rest.v2.models.error.Error(
                        code = 56, 
                        msg = '0', )
                    ]
            )
        else :
            return CalcTaxesResponse(
        )

    def testCalcTaxesResponse(self):
        """Test CalcTaxesResponse"""
        inst_req_only = self.make_instance(include_optional=False)
        inst_req_and_optional = self.make_instance(include_optional=True)


if __name__ == '__main__':
    unittest.main()
