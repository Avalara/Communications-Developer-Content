...
$FunctionPath = 'API', 'Model' | ForEach-Object {Join-Path "$ScriptDir\src\avalara.comms.rest.v2\" $_}
$BinPath = "$ScriptDir\src\avalara.comms.rest.v2\Bin"

Start-Process -FilePath "$ClientPath\build.bat" -WorkingDirectory $ClientPath -Wait -NoNewWindow

if (!(Test-Path "$ScriptDir\src\avalara.comms.rest.v2\Bin" -PathType Container)) {
    New-Item "$ScriptDir\src\avalara.comms.rest.v2\Bin" -ItemType Directory > $null
}

Copy-Item "$ClientPath\bin\*.dll" $BinPath

$Manifest = @{
    Path = "$ScriptDir\src\avalara.comms.rest.v2\avalara.comms.rest.v2.psd1"
    ...
    #
    # DefaultCommandPrefix = 'CustomizationApi'
    # DefaultCommandPrefix = 'HealthCheckApi'
    # DefaultCommandPrefix = 'JurisdictionDeterminationApi'
    # DefaultCommandPrefix = 'LookupsApi'
    # DefaultCommandPrefix = 'TaxCalculationApi'
}
