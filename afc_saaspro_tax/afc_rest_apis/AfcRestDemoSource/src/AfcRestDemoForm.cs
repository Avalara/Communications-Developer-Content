...
namespace Avalara.CommsPlatform.Api.RestDemoApplication
{
    ...
    public partial class AfcRestDemoForm : Form
    {
        ...
        private string geoBatchInputFileLink;
        private string geoBatchOutputFileLink;
                    ...
                    new Invoice
                    {
                        ...
                        Items = new []
                        {
                            new LineItem
                            {
                                ...
                                SaleType = (short)1, //SaleType.Retail,
        ...
        private void gotoGeoBatchCommsPlatform_Click(object sender, EventArgs e)
        {
            var geoBatchSite = $"{ ConfigurationManager.AppSettings["AfcRestApiUrl"]}/AFC/GeoBatch";
            ProcessStartInfo siteInfo = new ProcessStartInfo(geoBatchSite);
            Process.Start(siteInfo);
        }

        private void geoBatchInputDownload_Click(object sender, EventArgs e)
        {
            ProcessStartInfo siteInfo = new ProcessStartInfo(geoBatchInputFileLink);
            Process.Start(siteInfo);
        }

        private void geoBatchOutputDownload_Click(object sender, EventArgs e)
        {
            ProcessStartInfo siteInfo = new ProcessStartInfo(geoBatchOutputFileLink);
            Process.Start(siteInfo);
        }
    }
}
